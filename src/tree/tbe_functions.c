...
int cb_full_traversal_network(pll_unetwork_node_t * node)
{
  (void) node;
  return 1;
}
...
void postorder_init_recursive_network(pll_unetwork_node_t * node, unsigned int * index,
                              unsigned int * subtree_size, index_information_t* idx_infos)
{
  if (node->next == NULL)
  {
  ...
  do
  {
    postorder_init_recursive_network(snode->back, index, subtree_size, idx_infos);
    snode = snode->next;
  } while (snode && snode != node);
  index_information_t info;
  info.idx = node->clv_index;
...
void postorder_init_network(pll_unetwork_node_t * root, unsigned int * trav_size,
                    unsigned int * subtree_size, index_information_t* idx_infos)
{
  *trav_size = 0;
  postorder_init_recursive_network(root->back, trav_size, subtree_size, idx_infos);
  postorder_init_recursive_network(root, trav_size, subtree_size, idx_infos);
}
...
tbe_data_t* init_tbe_data_network(pll_unetwork_node_t * root, unsigned int tip_count)
{
  tbe_data_t* data = (tbe_data_t*) malloc(sizeof(tbe_data_t));
  data->tip_count = tip_count;
  data->tip_count_div_2 = tip_count / 2;
  ...
  data->idx_infos = (index_information_t*) malloc(sizeof(index_information_t) * data->nodes_count);
  data->count_ones = (unsigned int*) malloc(sizeof(unsigned int) * data->nodes_count);
  postorder_init_network(root, &data->trav_size, data->subtree_size, data->idx_infos);
  return data;
}
...
PLL_EXPORT
pllmod_tbe_split_info_t * pllmod_unetwork_tbe_nature_init(pll_unetwork_t * ref,
                                                       unsigned int tip_count,
                                                       const pll_unetwork_node_t** split_to_node_map)
{
  unsigned int nodes_count = 2 * tip_count - 2;
  unsigned int split_count = tip_count - 3;
  unsigned int subtree_size[nodes_count];
  ...
  pll_unetwork_node_t ** travbuffer = (pll_unetwork_node_t **) malloc(nodes_count * sizeof(pll_unetwork_node_t *));
  ...
  // do a post-order traversal of the reference tree.
  unsigned int trav_size;
  pll_unetwork_tree_traverse(ref, PLL_TREE_TRAVERSE_POSTORDER, cb_full_traversal_network, travbuffer, &trav_size, 0);
  for (unsigned int i = 0; i < trav_size; ++i)
  { // first, we compute the subtree sizes.
    unsigned int idx = travbuffer[i]->clv_index;
    if (travbuffer[i]->next == NULL)
...
PLL_EXPORT int pllmod_unetwork_tbe_nature(pll_split_t * ref_splits,
                                       pll_split_t * bs_splits,
                                       pll_unetwork_node_t* bs_root,
                                       unsigned int tip_count,
                                       double * support,
                                       pllmod_tbe_split_info_t* split_info)
{
  ...
  for (i = 0; i < split_count; i++)
  {
    ...
    if (!tbe_data)
      tbe_data = init_tbe_data_network(bs_root, tip_count);
