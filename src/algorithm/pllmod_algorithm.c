...
static int networkinfo_get_alpha(const pllmod_networkinfo_t * networkinfo,
                              unsigned int  part_num,
                              double * param_vals,
                              unsigned int param_count)
{
  if (part_num >= networkinfo->partition_count)
    return PLL_FAILURE;

  param_vals[0] = networkinfo->alphas[part_num];
  return PLL_SUCCESS;
}
...
static int networkinfo_set_alpha(pllmod_networkinfo_t * networkinfo,
                              unsigned int  part_num,
                              const double * param_vals,
                              unsigned int param_count)
{
  ...
  networkinfo->alphas[part_num] = param_vals[0];

  pll_partition_t * partition = networkinfo->partitions[part_num];

  /* update rate categories */
  if (!pll_compute_gamma_cats (networkinfo->alphas[part_num],
                               partition->rate_cats,
                               partition->rates,
							   networkinfo->gamma_mode[part_num]))
    return PLL_FAILURE;
...
static int networkinfo_get_pinv(const pllmod_networkinfo_t * networkinfo,
                             unsigned int  part_num,
                             double * param_vals,
                             unsigned int param_count)
{
  ...
  pll_partition_t * partition = networkinfo->partitions[part_num];
  param_vals[0] = partition->prop_invar[networkinfo->param_indices[part_num][0]];
  return PLL_SUCCESS;
}
...
static int networkinfo_set_pinv(pllmod_networkinfo_t * networkinfo,
                             unsigned int  part_num,
                             const double * param_vals,
                             unsigned int param_count)
{
  ...
  {
    if (!pll_update_invariant_sites_proportion(partition,
    		networkinfo->param_indices[part_num][k],
                                          param_vals[0]))
      return PLL_FAILURE;
  }
...
static int networkinfo_get_brlen_scaler(const pllmod_networkinfo_t * networkinfo,
                                     unsigned int part_num,
                                     double * param_vals,
                                     unsigned int param_count)
{
  ...
  param_vals[0] = networkinfo->brlen_scalers[part_num];
  return PLL_SUCCESS;
}
...
static int networkinfo_set_brlen_scaler(pllmod_networkinfo_t * networkinfo,
                                     unsigned int part_num,
                                     const double * param_vals,
                                     unsigned int param_count)
{
  ...
  networkinfo->brlen_scalers[part_num] = param_vals[0];
...
static void fix_brlen_scalers_network(pllmod_networkinfo_t * networkinfo,
                              double min_scaler,
                              double max_scaler)
{
  unsigned int i;

  assert(networkinfo->brlen_scalers);

  double lowest_scaler  = networkinfo->brlen_scalers[0];
  double highest_scaler = networkinfo->brlen_scalers[0];
  ...
  if (networkinfo->parallel_reduce_cb)
  {
    for (i = 0; i < networkinfo->partition_count; ++i)
    {
      if (!networkinfo->partitions[i])
    	  networkinfo->brlen_scalers[i] = 0.0;
    }

    networkinfo->parallel_reduce_cb(networkinfo->parallel_context,
    		                        networkinfo->brlen_scalers,
			                        networkinfo->partition_count,
                                 PLLMOD_COMMON_REDUCE_MAX);
  }
  ...
  for (i = 0; i < networkinfo->partition_count; ++i)
  {
    if (networkinfo->brlen_scalers[i] < lowest_scaler)
      lowest_scaler = networkinfo->brlen_scalers[i];
    if (networkinfo->brlen_scalers[i] > highest_scaler)
      highest_scaler = networkinfo->brlen_scalers[i];
  }
  ...
  if (lowest_scaler < min_scaler || highest_scaler > max_scaler)
  {
    ...
    /* force scalers into bounds by multiplying with correction factor */
    for (i = 0; i < networkinfo->partition_count; ++i)
    	networkinfo->brlen_scalers[i] *= global_scaler;

    /* multiply all branches by the inverse to preserve likelihood */
    pllmod_networkinfo_scale_branches_all(networkinfo, 1.0 / global_scaler);
  }
}
  ...
  if (param_count > 0)
  {
    ...
    for (i = 0; i < networkinfo->partition_count; ++i)
    {
      if (networkinfo->params_to_optimize[i] & param_to_optimize)
      {
        ...
        params_getter(networkinfo, i, &param_vals[j], 1);
        opt_mask[j] = 1;
        j++;
      }
    }
    ...
    struct networkinfo_opt_params opt_params;
    opt_params.networkinfo        = networkinfo;
    opt_params.param_to_optimize  = param_to_optimize;
    opt_params.num_opt_partitions = param_count;
    opt_params.param_set_cb       = params_setter;
    ...
    int ret = pllmod_opt_minimize_brent_multi(param_count,
                                              ...
                                              NULL, NULL,  /* fx, f2x */
                                              (void *) &opt_params,
                                              &target_func_onedim_networkinfo,
                                              1 /* global_range */
                                              );
                                                  ...
                                                  double tolerance)
{
  if (__builtin_popcount(param_to_optimize) > 1)
  {
    pllmod_set_error(PLL_ERROR_PARAM_INVALID,
                     "Multi-parameter optimization is not supported by the "
                     "pllmod_algo_opt_onedim_networkinfo() function!");
    return -INFINITY;
  }

  networkinfo_param_get_cb params_getter = NULL;
  networkinfo_param_set_cb params_setter = NULL;

  switch (param_to_optimize)
  {
    case PLLMOD_OPT_PARAM_ALPHA:
      params_getter = networkinfo_get_alpha;
      params_setter = networkinfo_set_alpha;
      break;
    case PLLMOD_OPT_PARAM_PINV:
      params_getter = networkinfo_get_pinv;
      params_setter = networkinfo_set_pinv;
      break;
    case PLLMOD_OPT_PARAM_BRANCH_LEN_SCALER:
      params_getter = networkinfo_get_brlen_scaler;
      params_setter = networkinfo_set_brlen_scaler;
      break;
    default:
      pllmod_set_error(PLL_ERROR_PARAM_INVALID, "Unsupported parameter: %d",
                       param_to_optimize);
  ...
  return pllmod_algo_opt_onedim_networkinfo_custom(networkinfo,
                                                param_to_optimize,
                                                params_getter,
                                                params_setter,
                                                min_value,
                                              ...
                                              double lh_epsilon)
{
  ...
  old_scalers = (double *) calloc(networkinfo->init_partition_count,
                                  sizeof(double));
  old_brlen   = (double *) calloc(networkinfo->network->edge_count,
                                  sizeof(double));
  for (i = 0, j = 0; i < networkinfo->partition_count; ++i)
  {
    if (networkinfo->partitions[i])
      old_scalers[j++] = networkinfo->brlen_scalers[i];
  }
  assert(j == networkinfo->init_partition_count);

  memcpy(old_brlen, networkinfo->branch_lengths[0],
		  networkinfo->network->edge_count * sizeof(double));

  /* make sure all brlen scalers are between min_scaler and max_scaler */
  fix_brlen_scalers_network(networkinfo, min_scaler, max_scaler);

  loglh = -1 * pllmod_algo_opt_onedim_networkinfo(networkinfo,
                                          PLLMOD_OPT_PARAM_BRANCH_LEN_SCALER,
                                          min_scaler,
                                          max_scaler,
                                          lh_epsilon);
  ...
  /* normalize scalers and scale the branches accordingly */
  pllmod_networkinfo_normalize_brlen_scalers(networkinfo);
  ...
  for (i = 0; i < networkinfo->subnode_count; ++i)
  {
    ...
    {
      pllmod_networkinfo_set_branch_length(networkinfo, snode, max_brlen);
      brlen_fixed = 1;
    }
  }
  ...
  if (brlen_fixed)
  {
    ...
    if (loglh < old_loglh)
    {
      ...
      {
        if (networkinfo->partitions[i])
        	networkinfo->brlen_scalers[i] = old_scalers[j++];
      }
      assert(j == networkinfo->init_partition_count);
                                             ...
                                             double tolerance)
{
  ...
  for (i = 0; i < networkinfo->partition_count; ++i)
  {
    if (networkinfo->params_to_optimize[i] & PLLMOD_OPT_PARAM_SUBST_RATES)
    {
      part_count++;
      ...
      size_t nrates = pllmod_util_subst_rate_count(networkinfo->partitions[i]->states);
      if (nrates > max_free_params)
        max_free_params = nrates;
    }
  }
  ...
  {
    double tmp = (double) max_free_params;
    networkinfo->parallel_reduce_cb(networkinfo->parallel_context, &tmp, 1,
                                 PLLMOD_COMMON_REDUCE_MAX);
    max_free_params = (size_t) tmp;
  }
  ...
  for (i = 0; i < networkinfo->partition_count; ++i)
  {
    ...
    unsigned int states    = partition->states;
    unsigned int subst_params = pllmod_util_subst_rate_count(states);
    int * symmetries = networkinfo->subst_matrix_symmetries[i];
                                             ...
                                             double tolerance)
{
  ...
  for (i = 0; i < networkinfo->partition_count; ++i)
  {
    if (networkinfo->params_to_optimize[i] & PLLMOD_OPT_PARAM_FREQUENCIES)
    {
      ...
      size_t nfree_params = networkinfo->partitions[i]->states - 1;
      if (nfree_params > max_free_params)
        max_free_params = nfree_params;
    }
  }
...
static void scales_rates_and_branches_network(pllmod_networkinfo_t * networkinfo,
                                      size_t part_num,
                                      double rate_scaler)
{
  assert(networkinfo);
  assert(part_num < networkinfo->partition_count);
  assert(rate_scaler > 0.);
  ...
  /* scale branch lengths such that likelihood is conserved */
  if (networkinfo->partition_count == 1)
    pllmod_networkinfo_scale_branches_all(networkinfo, brlen_scaler);
  else if (networkinfo->brlen_linkage == PLLMOD_COMMON_BRLEN_UNLINKED)
    pllmod_networkinfo_scale_branches_partition(networkinfo, part_num, brlen_scaler);
  else
  {
    assert(networkinfo->brlen_scalers);

    /* update brlen scalers */
    networkinfo->brlen_scalers[part_num] *= brlen_scaler;
  }
}
...
static void fix_free_rates_network(pllmod_networkinfo_t * networkinfo,
                           double min_rate,
                           double max_rate)
{
  size_t i,j;
  ...
  for (i = 0; i < networkinfo->partition_count; ++i)
  {
    /* skip remote partitions and those without rates/weight optimization */
    if (!networkinfo->partitions[i] ||
    	!(networkinfo->params_to_optimize[i] & PLLMOD_OPT_PARAM_FREE_RATES))
      continue;
    ...
    if (lowest_rate < min_rate || highest_rate > max_rate)
    {
      ...
      scales_rates_and_branches_network(networkinfo, i, rate_scaler);
    }
  }
}
                                               ...
                                               double tolerance)
{
  ...
  for (i = 0; i < networkinfo->partition_count; ++i)
  {
    if (networkinfo->params_to_optimize[i] &
        (PLLMOD_OPT_PARAM_FREE_RATES | PLLMOD_OPT_PARAM_RATE_WEIGHTS))
    {
      part_count++;
      ...
      size_t nfree_params = networkinfo->partitions[i]->rate_cats;
      if (nfree_params > max_free_params)
        max_free_params = nfree_params;
    }
  }
  ...
  do
  {
    ...
    for (i = 0; i < networkinfo->partition_count; ++i)
    {
      if (networkinfo->params_to_optimize[i] & PLLMOD_OPT_PARAM_RATE_WEIGHTS)
      {
        pll_partition_t * partition = networkinfo->partitions[i];
    ...
    for (i = 0; i < networkinfo->partition_count; ++i)
    {
      if (networkinfo->params_to_optimize[i] & PLLMOD_OPT_PARAM_FREE_RATES)
      {
        pll_partition_t * partition = networkinfo->partitions[i];
  ...
  {
    /* skip remote partitions and those without rates/weight optimization */
    if (!networkinfo->partitions[i] || !(networkinfo->params_to_optimize[i] &
        (PLLMOD_OPT_PARAM_FREE_RATES | PLLMOD_OPT_PARAM_RATE_WEIGHTS)))
      continue;
...
double pllmod_algo_opt_brlen_networkinfo(pllmod_networkinfo_t * networkinfo,
                                      ...
                                      int radius)
{
  return pllmod_opt_optimize_branch_lengths_local_multi_network(networkinfo->partitions,
		                                                networkinfo->partition_count,
		                                                networkinfo->root,
		                                                networkinfo->param_indices,
		                                                networkinfo->deriv_precomp,
		                                                networkinfo->branch_lengths,
		                                                networkinfo->brlen_scalers,
                                                        min_brlen,
                                                        max_brlen,
                                                        lh_epsilon,
                                                        ...
                                                        1,    /* keep_update */
                                                        opt_method,
														networkinfo->brlen_linkage,
														networkinfo->parallel_context,
														networkinfo->parallel_reduce_cb
                                                        );
